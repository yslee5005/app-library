...
-- profiles = 공통 auth (packages/auth 관리)
-- abba_user_settings = 앱 전용 도메인 데이터
-- ============================================================

-- =====================
-- 1. Profiles (공통 — 모든 앱 공유)
-- packages/auth가 관리하는 인증 프로필
-- =====================
CREATE TABLE IF NOT EXISTS profiles (
  ...
  email TEXT,
  avatar_url TEXT,
  provider TEXT,                    -- 'google', 'apple', 'email', 'anonymous'
...
-- =====================
-- 2. Abba User Settings (abba 전용)
-- 기도 통계, 구독, 언어, 음성 등 앱 도메인 데이터
-- =====================
...
CREATE INDEX idx_abba_settings_user ON abba_user_settings(user_id);
...
-- =====================
-- Auto-create profile + settings on signup (trigger)
-- =====================
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  -- 공통 profile (packages/auth 호환)
  INSERT INTO public.profiles (id, app_id, display_name, email, provider)
  VALUES (
    NEW.id,
    COALESCE(NEW.raw_app_meta_data->>'app_id', 'abba'),
    COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1)),
    COALESCE(NEW.email, ''),
    COALESCE(NEW.raw_app_meta_data->>'provider', 'anonymous')
  );

  -- abba 전용 settings (조건부)
  IF COALESCE(NEW.raw_app_meta_data->>'app_id', 'abba') = 'abba' THEN
