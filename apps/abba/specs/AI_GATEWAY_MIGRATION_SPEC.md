...
### Strategy B = 모드별 단일 캐시 (v1 default, 출시 후 데이터로 재평가)
...
### v1에서 B를 채택하는 이유 (출시 시점 기준)
- 출시 전엔 트래픽 0 → locale 분포·hot/cold 패턴 측정 불가 → 추측 결정 회피
- B는 storage 비용 1/35 (cache 1개), rubric invalidation 1회로 끝
- 모든 locale 첫 사용자가 hot cache 즉시 받음 (한·영 활동만 있어도 base cache가 keep-alive)
- A(locale별 35 cache) 옵션은 운영 데이터로 결정 — 아래 트리거

### Strategy A 마이그레이션 트리거 (출시 +1개월 첫 평가, 이후 분기별)
다음 중 하나라도 충족하면 locale별 35 cache로 마이그레이션 검토:
1. **Minority locale latency 악화** — 특정 locale (sw/am/my 등) cache miss 비율 30%+ → 그 locale 첫 사용자가 매번 cold start
2. **rubric 핫패치 빈도** — 운영 중 rubric 변경 주 1회+ 시 invalidation 1회의 가치 ↓ (35회로 분산이 더 안전)
3. **inline locale policy 비용 누수** — 월 inline overhead가 storage 차이($9/월 - $0.07/월 = $8.93/월) 명확히 초과
4. **cache hit ratio 비대칭** — 한·영 90%+ 인데 minority locale 60% 이하 (UX 격차)

→ 측정 없이 임의 마이그레이션 X. v1은 무조건 B로 시작.
