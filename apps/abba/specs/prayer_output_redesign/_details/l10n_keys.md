...
| Key | EN | KO | 용도 / INT |
|---|---|---|---|
| `aiPrayerCitationsTitle` | `References · Citations` | `참고 · 인용` | citations 섹션 제목 (INT-045) |
| `citationTypeQuote` | `Quote` | `명언` | quote 타입 라벨 (INT-045) |
| `citationTypeScience` | `Research` | `연구` | science 타입 라벨 (INT-045) |
| `citationTypeExample` | `Example` | `예시` | example 타입 라벨 (INT-045) |
| `aiPrayerReadingTime` | `2-minute read` | `2분 읽기` | 카드 제목 옆 뱃지 (선택, INT-045) |

### namedArgs 정의

namedArgs 없음. 모두 고정 텍스트.

### 35 locale 전략

- 1차: en + ko 정확
- 2차 (Phase 5 구현 말미): 나머지 33 언어 — Phase 3 때 만든 `/tmp/add_phase*_l10n.py` 스크립트 패턴 재사용

### 언어별 초안 (주요 locale)

| Locale | citationsTitle | quote | science | example | readingTime |
|--------|----------------|-------|---------|---------|-------------|
| en | References · Citations | Quote | Research | Example | 2-minute read |
| ko | 참고 · 인용 | 명언 | 연구 | 예시 | 2분 읽기 |
| ja | 参照 · 引用 | 名言 | 研究 | 例 | 2分で読める |
| zh | 参考 · 引用 | 名言 | 研究 | 例子 | 2分钟阅读 |
| es | Referencias · Citas | Cita | Estudio | Ejemplo | 2 minutos de lectura |
| fr | Références · Citations | Citation | Étude | Exemple | Lecture de 2 min |
| de | Quellen · Zitate | Zitat | Studie | Beispiel | 2-Minuten-Lektüre |
