...
### 6. Supabase Storage RLS — `TO public` 사용 (결정)

Supabase RLS role 용어 정리:
- `anon` — JWT 없는 요청 (완전 비로그인)
- `authenticated` — JWT 있는 모든 유저 (**익명 auth 포함**)
- `public` — `anon` + `authenticated` 둘 다

**Abba 결정 (사용자 확인 2026-04-21)**: bibles/* 는 **`TO public`** 사용.
- 이유: PD Bible 콘텐츠에 민감정보 0
- Anonymous-First 구조상 대부분 유저가 JWT 보유하나, `signInAnonymously()` 실패 edge case (네트워크 이슈, JWT 만료 transient) 대응
- 제한할 실익 없고 제한 시 edge case만 늘어남

UPDATE/INSERT/DELETE는 **service_role만** 자동 허용 (RLS가 authenticated/anon에 default deny → 업로드는 Supabase Dashboard나 service_role key 필요).
