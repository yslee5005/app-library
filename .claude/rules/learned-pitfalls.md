...
- **main.dart DI 업데이트 누락 — mock 테스트는 통과, 실기만 터지는 회귀**: 새 서비스(`GeminiService`)에 required dependency(`GeminiCacheManager`, `BibleTextService`)를 생성자 인자로 추가했는데 `main.dart` override 체인에서 `GeminiService()` 를 여전히 인자 없이 호출 → 실기에서 `analyzePrayerStreamed` / `analyzeMeditationStreamed` / `analyzeTier3Prayer` 가 null check 에서 즉시 `TierFailed(apiError, "DI not wired")` 반환. 기존 `analyzePrayer*` Future 계열은 이 필드 안 써서 영향 없음. `flutter analyze` 는 명시적 null 처리라서 경고 안 냄. `flutter test` 는 `MockAiService` 가 이 경로 커버 안 함. **결과: Phase 4.1/4.2 streaming 기능 전체가 실기에서 한 번도 동작 안 한 상태**로 INT-048 자동 검증까지 통과. **예방**: (a) 새 service에 required dependency 추가 시 **main.dart real 모드 + mock 모드 override 블록 둘 다** 업데이트 체크, (b) null check fallback path는 반드시 **구체적 원인 메시지**를 포함 (`'DI not wired' → 'GeminiService missing cacheManager: did main.dart wire it?'`), (c) `AiAnalysisException` 같은 wrapper throw 시 `const` 생성자는 `cause` 를 못 실음 → ai_loading_view 의 catch 에서 `e.message` 도 로그에 포함해야 조용한 실패 방지.
